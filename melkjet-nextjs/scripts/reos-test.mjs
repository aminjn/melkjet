...
// ─── فاز ۴۵ (سند ۲۹ — Auction Saga): تالارِ مزایدهٔ هفته ───
{
  ...
  // فاز ۱۰۳ (جلد ۳ — Prestige): اثرهای شفافِ درختِ مهارت + ضریبِ درآمد
  {
    ...
    // ردِ خودکار فقط با مدرکِ قطعیِ تماس (با نقلِ همان تکه)؛ ML هرگز خودش رد نمی‌کند.
    {
      ...
      const a138 = await getItemById('a138'); const b138 = await getItemById('b138')
      ok('بازممیزی فقط ردهای خودکار را برمی‌گرداند (۱ از ۲) و ردِ دستی می‌ماند', n138 === 1 && a138.status === 'pending' && !a138.moderatedAt && b138.status === 'rejected')
