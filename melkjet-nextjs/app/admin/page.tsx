        ...
        {scrapes.map(s => (
          <div key={s.id} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              ...
              {s.running && <span style={{ fontSize: 11, padding: '2px 9px', borderRadius: 999, background: 'rgba(201,168,76,0.15)', color: 'var(--gold)', fontWeight: 700 }}>{s.phase === 'import' ? 'در حال واردکردنِ آگهی‌ها از دیوار…' : 'در حال خواندنِ فهرست…'} {s.progress?.total ? `${fa(s.progress.done)}/${fa(s.progress.total)}` : ''}</span>}
