...
// آیتم‌های تقویم از نوبت‌ها.
const apptCalItems = (requests: ProRequest[]) => requests.filter(r => r.meta?.jdate).map(r => ({ id: r.id, dateKey: r.meta!.jdate as string, time: r.meta?.time as string | undefined, label: r.clientName, color: N_COLOR[r.status] }))
  ...
  return (
    <Modal title={`ثبتِ سند — ${req.clientName}`} onClose={onClose}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <FileField value={f.file} onChange={v => setF({ ...f, file: v })} label="📎 پیوستِ فایلِ سند (تصویر یا PDF)" />
      </div>
      ...
      {!data ? <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--muted)' }}>در حال بارگذاری…</div> :
        view === 'dashboard' ? (
          <>
            ...
            <ProCalendar items={apptCalItems(data.requests)} today={today} title="تقویمِ نوبت‌ها" />
