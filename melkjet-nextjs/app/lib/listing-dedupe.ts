...
export async function dedupeListings(): Promise<{ removed: number; kept: number }> {
  ...
  // فاز ۱۵۶ (A3): اولویتِ «ماندن» دیگر فقط قدمت نیست — چون آپدیتِ درجای اسکرپِ مجدد scrapedAt را
  // تازه می‌کند و «کهنه می‌ماند» یعنی نسخهٔ به‌روز قربانی می‌شد. ترتیبِ ماندن:
  // تأییدشده (حکمِ ادمین/ممیزی) → دارای مهرِ معاملهٔ واقعی (__soldAt) → قدیمی‌تر.
  const rank = (i: Item) => (i.status === 'approved' ? 0 : 1)
  const soldRank = (i: Item) => (i.meta?.['__soldAt'] ? 0 : 1)
  items.sort((a, b) => rank(a) - rank(b) || soldRank(a) - soldRank(b) || (a.scrapedAt || 0) - (b.scrapedAt || 0))
  const blocks = new Map<string, Fields[]>()
  const dupIds: string[] = []
  let kept = 0
  for (const it of items) {
    // مهرِ معاملهٔ واقعی = سابقهٔ معامله، نه آگهیِ اضافی: نه خودش تکراری می‌شود،
    // نه مبنای تکراری‌کردنِ آگهیِ فعالِ بعدیِ همان ملک قرار می‌گیرد.
    if (it.meta?.['__soldAt']) { kept++; continue }
    const f = fieldsOf(it)
