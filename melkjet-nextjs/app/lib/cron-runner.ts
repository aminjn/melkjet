...
declare global {
  ...
  var __mjCron: { started: boolean; running: boolean; runningSince?: number } | undefined
}

// 🐶 فاز ۱۸۸ — سگِ نگهبانِ قفل (ریشهٔ «کارگر ۳۰ ساعت تیک نزد»): اگر یکی از awaitهای بدنهٔ تیک
// (فراخوانِ AI/شبکه/PG بدونِ timeout) برای همیشه هنگ کند، قفلِ سراسری آزاد نمی‌شد و همهٔ
// تیک‌های بعدی در خطِ اول برمی‌گشتند — کرون تا ری‌استارت مرده بود. حالا قفلِ ماندهٔ بیش از
// سقف، با لاگِ خطا شکسته می‌شود و چرخه خودش را ترمیم می‌کند. (تیکِ سالم هرگز این‌قدر طول نمی‌کشد.)
...
async function tick(): Promise<{ due: number; synced: number }> {
  ...
  if (!g) return { due: 0, synced: 0 }
  if (g.running) {
    if (!lockStuck(g.runningSince, Date.now(), TICK_STUCK_MS)) return { due: 0, synced: 0 }
    console.error(`[cron] tick lock stuck since ${new Date(g.runningSince || 0).toISOString()} — force releasing (self-heal)`)
  }
  g.running = true
...
const MAX_ACTIVE_SYNCS = 4          // حداکثر همگام‌سازیِ همزمان روی کلِ سیستم — فاز ۱۳۴: با ۷۰+ مشاورِ روزانه، ۲ صف را ساعتی می‌کرد (کارها network-boundاند)
const QUEUE_TICK_MS = 45 * 1000     // هر ۴۵ ثانیه صف را درین کن (پاسخ‌گوییِ خوب به کاربر)
let queueRunning = false
let queueRunningSince = 0
...
async function queueTick(): Promise<void> {
  if (queueRunning) {
    if (!lockStuck(queueRunningSince, Date.now(), QUEUE_STUCK_MS)) return
    console.error(`[cron] queueTick lock stuck since ${new Date(queueRunningSince).toISOString()} — force releasing (self-heal)`)
  }
  queueRunning = true
  queueRunningSince = Date.now()
  try {
    ...
    // فاز ۱۷۶ (پورت از خودروجت) — ضربانِ کرونِ قابلِ‌خواندن از بیرون: اثباتِ زنده‌بودنِ کرون + آخرین سینکِ رُستر
    try { const { touchCron } = await import('./cron-heartbeat'); await touchCron({ instance: process.env.NODE_APP_INSTANCE }) } catch {}
    ...
    //    فاز ۱۸۸ — fire-and-forget: فراخوانِ AI دیگر تیک را نگه نمی‌دارد (ریشهٔ هنگِ ۳۰ساعتهٔ کارگر —
    //    یک fetch بی‌timeout قفل را برای همیشه گرفته بود). قفلِ moderating فقط از هم‌پوشانی جلوگیری می‌کند
    //    و اگر خودش گیر کند، فقط ممیزی عقب می‌افتد نه ضربان/صفِ اسکرپ.
    if (!moderating) {
      moderating = true
      moderatingSince = Date.now()
      ;(async () => {
    ...
    } else if (lockStuck(moderatingSince, Date.now(), 60 * 60_000)) {
      console.error('[cron] moderatePending stuck >60min — releasing lock so next tick retries')
      moderating = false
    }
    ...
    //    فاز ۱۷۶ (پورت از خودروجت): «ساختِ حساب» (graduate) قفلِ جداگانه دارد تا پشتِ سینکِ چنددقیقه‌ای گیر نکند.
    if (!graduateRunning) {
      graduateRunning = true
      ;(async () => {
        try {
    ...
    if (!rosterRunning) {
      ...
      ;(async () => {
        try {
          ...
          if (due.length) {
            await touchCron({ lastRosterDueAt: Date.now(), lastRosterSyncAt: Date.now(), lastRosterSyncSlug: due[0].slug })
            await syncRoster(due[0].id)
          }
