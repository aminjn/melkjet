...
import { readJsonCached, writeJsonCached, withFileLock, readJsonFresh } from './json-file'
...
// فاز ۲۴۱ (ریشهٔ «رفعِ تعلیق می‌زنم، دوباره معلق می‌شود»): ۴ اینستنسِ pm2 بدونِ قفل روی همین فایل
// load→mutate→save می‌کردند و نوشتهٔ دیرتر تغییراتِ زودتر را کامل می‌بلعید (کرانِ گیتِ پروفایل در
// یک پاس صدها نوشتن دارد — نوشتهٔ ادمین وسطِ پاس قربانی می‌شد؛ در طولِ روزها پرچم‌های ماندگار هم
// همین‌طور گم می‌شدند). از این پس «همهٔ» نوشتن‌ها زیرِ قفلِ بین‌پروسه‌ای + خواندنِ تازه از دیسک‌اند.
function mutateDb<R>(fn: (db: DB) => R): R {
  return withFileLock(FILE, () => { const db = readJsonFresh<DB>(FILE, {}); const r = fn(db); save(db); return r })
}
...
export function touchLogin(phone: string) { mutateDb(db => { if (db[phone]) db[phone].lastLogin = Date.now() }) }
...
export function setGateExempt(phone: string, on: boolean) {
  mutateDb(db => { const a = db[phone]; if (a) a.gateExempt = on || undefined })
}
// فاز ۲۴۱ — تعلیق/رفعِ تعلیقِ ادمین در «یک» نوشتنِ اتمیک: رفعِ تعلیق = معافیتِ ماندگار در همان لحظه؛
// دیگر پنجره‌ای بینِ دو نوشتنِ جدا نیست که کرانِ گیت وسطش بنویسد و هر دو را ببلعد.
export function adminSetSuspension(phone: string, suspend: boolean, reason?: string): boolean {
  return mutateDb(db => {
    const a = db[phone]; if (!a) return false
    a.suspended = suspend || undefined
    a.suspendReason = suspend ? (reason || a.suspendReason) : undefined
    a.suspendedAt = suspend ? Date.now() : undefined
    a.gateExempt = suspend ? undefined : true
    if (!suspend) a.profileWarnAt = undefined
    if (suspend) { a.flagged = undefined; a.flagReason = undefined; a.flaggedAt = undefined }
    return true
  })
}
// فاز ۲۴۱ — تعلیقِ خودکارِ «شرطی» برای کرانِ گیت: شرط‌ها زیرِ همان قفلِ نوشتن دوباره چک می‌شوند —
// اگر ادمین همین الان رفعِ تعلیقِ ماندگار زده باشد (gateExempt)، تعلیق انجام نمی‌شود (پایانِ «۲ ثانیه بعد معلق شد»).
export function gateSuspendIfEligible(phone: string, graceMs: number, now = Date.now()): boolean {
  return mutateDb(db => {
    const a = db[phone]; if (!a) return false
    if (a.gateExempt || a.suspended) return false
    if (!a.profileWarnAt || now - a.profileWarnAt <= graceMs) return false
    a.suspended = true; a.suspendedAt = now
    a.flagged = undefined; a.flagReason = undefined; a.flaggedAt = undefined
    return true
  })
}
...
export function setProfileWarn(phone: string, ts: number | undefined) { mutateDb(db => { const a = db[phone]; if (a) a.profileWarnAt = ts }) }
export function deleteAccount(phone: string) { mutateDb(db => { delete db[phone] }) }
...
export function bulkDelete(phones: string[]) { mutateDb(db => { for (const p of phones) delete db[p] }) }
