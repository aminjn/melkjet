...
interface WfState { lastRun: number; done: string[]; stages?: Record<string, string> }
...
export function getWfState(id: string): WfState { return load()[id] || { lastRun: 0, done: [], stages: {} } }
export function setWfState(id: string, st: WfState) {
  const db = load()
  db[id] = { lastRun: st.lastRun, done: (st.done || []).slice(-1000), stages: st.stages || {} }   // سقفِ تاریخچه
  save(db)
}
// با فعال‌سازیِ مجدد، وضعیت ریست می‌شود تا رویدادهای قدیمی دوباره شلیک نشوند.
export function resetWfState(id: string, now: number) { setWfState(id, { lastRun: now, done: [], stages: {} }) }
