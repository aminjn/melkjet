...
export async function GET(req: NextRequest) {
  ...
  const res = publicCatalogQuery({
    ...
    sort: u.get('sort') || undefined, page: num('page'), pageSize: num('pageSize'),
  })
  const counts = sellerCountsByCatalog()
  let items = res.items.map(p => ({ ...p, sellerCount: counts[p.id] || 0, sourceLabel: SOURCE_LABEL[p.source] || p.source }))
  if (u.get('withSeller') === '1') items = items.filter(p => p.sellerCount > 0)
