...
import { importDivarToken, syncAdvisorDivar } from '@/app/lib/advisor-divar-import'
...
export async function POST(req: NextRequest) {
  ...
  switch (b.action as string) {
    case 'importUrl': {
      ...
      const r = await importDivarToken(o, String(b.url))
      if (!r.ok) return NextResponse.json({ error: r.reason || 'ایمپورت ناموفق بود' }, { status: 400 })
      return NextResponse.json({ ok: true, skipped: !!r.skipped, listing: r.listing, config: getDivar(o) })
    }
    case 'sync': {
