      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '24px 20px 60px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <Link href="/stores" style={topLink}>🏪 فروشگاه‌ها ↗</Link>
            <Link href="/materials-prices" style={topLink}>📊 نرخِ روز ↗</Link>
          </div>
        </div>
            ...
            {items.length === 0 && !loading ? (
              ...
              <>
                <div className="mjb-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(190px,1fr))', gap: 16 }}>
                  {items.map(p => (
                    <Link key={p.id} href={`/product/${p.id}`} className="mjd-card" style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, overflow: 'hidden', textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', transition: 'transform .15s, box-shadow .15s' }}>
