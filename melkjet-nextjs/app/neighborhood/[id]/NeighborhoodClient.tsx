...
export default function NeighborhoodClient({ name }: { name: string }) {
  const neighborhoodName = name || 'saadatabad';
  ...
  useEffect(() => {
    ...
    const q = `city=${encodeURIComponent(neighborhoodName)}&district=${encodeURIComponent(neighborhoodName)}`;
    ...
    fetch(`/api/advisors/by-area?area=${encodeURIComponent(neighborhoodName)}`, { cache: 'no-store' })
