...
### REOS v2 — فاز ۷: CRM OS یکپارچه — اجرا شد
`app/lib/reos/crm.ts` (هستهٔ واحد، **افزایشی**؛ Sales-OSهای per-role دست‌نخورده): pipeline (۷ مرحله)،
activities، tasks، meetings، **timelineِ ادغام‌شده**، **funnelِ تبدیل** (نرخِ conversion)، و
**موتورِ automation** (تریگرهای new_lead/stage_change/idle → ساختِ تسک/جابه‌جاییِ مرحله/یادداشت).
امتیازِ لید از مدلِ `predictLeadConversion` می‌آید و با هر فعالیت/مرحله بازمحاسبه می‌شود. طرحِ
تک‌جدولی (`reos_crm`) کم‌ریسک، dual-mode. اتوماسیونِ idle در cron هر ۶ ساعت. مسیر: `/api/reos/crm`.
۹ تستِ PG. → **هر ۱۰ هدفِ ممیزی اجرا و تست شد.**

**تست‌های کل: ۴۳ واحد + ۷۰ روی PostgreSQLِ واقعی = ۱۱۳.**

> REOS v2 کامل شد (۱۰/۱۰). این یک لایهٔ **افزایشی** است: هستهٔ v1 و Sales-OSهای موجود دست‌نخورده‌اند
> و همه‌چیز backward-compatible است (بدونِ DATABASE_URL هم مثلِ قبل روی فایل کار می‌کند).
